...
const Eidt: React.FC<IndexProps> = ({ visible, onClose, data }) => {
  const [form] = useForm();
  const formLayout = {
    labelCol: { span: 16 },
    wrapperCol: { span: 8 },
  ...
  return (
    ...
    >
      <Form form={form} onFinish={onSubmit} {...formLayout}>
        ...
        <Row>
          <Col span={11}>
            <Item label="过关红包区间(元)" name="redEnvelopeMin" rules={[{ required: true, message: '请输入' }]}>
              <InputNumber placeholder="请输入" />
            </Item>
          </Col>
          <Col span={2} className={style.textCenter}>——</Col>
          <Col span={11}>
            <Item name="redEnvelopeMax" rules={[{ required: true, message: '请输入' }]}>
              <InputNumber placeholder="请输入" />
            </Item>
          </Col>
        </Row>
